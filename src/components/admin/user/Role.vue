...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      listPerms () {
        var _this = this
        this.$axios.get('/api/admin/role/perm').then(resp => {
      ...
      commitStatusChange (value, role) {
        if (role.id !== 1) {
          this.$confirm('是否更改角色状态？', '提示', {
            ...
            this.$axios.put('/api/admin/role/status', {
              enabled: value,
              id: role.id
            }).then(resp => {
              if (resp && resp.data.code === 200) {
    ...
    editRole (role) {
      ...
      onSubmit (role) {
        ...
        this.$axios.put('/api/admin/role/menu?rid=' + role.id, {
